# LLMRF 项目文档

**ADS 自动化仿真系统 - Python 命令行版本**

版本：2.0  
最后更新：2026-02-02

---

## 📁 项目结构

```
adsapi/
├── config.json          # 配置文件（唯一需要修改）
├── main.py             # 主程序入口
├── ads_engine.py       # ADS 仿真引擎
├── netlist_parser.py   # 网表解析器
├── visualizer.py       # 结果可视化
├── llm_interface.py    # LLM 接口
├── run_simple.py       # 简化启动器
├── check_config.py     # 配置检查工具
└── 项目文档.md         # 本文件
```

---

## 🚀 快速开始

### 1. 配置（只需一次）

编辑 `config.json`：

```json
{
  "ads": {
    "install_path": "F:/Program Files (x86)/ADS2026",
    "python_exe": "F:/Program Files (x86)/ADS2026/tools/python/python.exe"
  }
}
```

**检查配置**：
```cmd
"F:/Program Files (x86)/ADS2026/tools/python/python.exe" check_config.py
```

### 2. 运行仿真

**方式 1：使用启动器（推荐）**
```cmd
"F:/Program Files (x86)/ADS2026/tools/python/python.exe" run_simple.py
```

**方式 2：直接命令行**
```cmd
"F:/Program Files (x86)/ADS2026/tools/python/python.exe" main.py --auto
```

**方式 3：指定工作空间**
```cmd
"F:/Program Files (x86)/ADS2026/tools/python/python.exe" main.py ^
  --workspace "D:\Desktop\test_wrk" ^
  --library "test_lib" ^
  --design "test"
```

---

## 📋 核心功能

### 1. ADS 仿真引擎 (`ads_engine.py`)

**功能**：
- 连接 ADS 工作空间和设计
- 生成和修改网表
- 运行仿真（使用 CircuitSimulator）
- 提取结果数据（.ds 文件 → DataFrame）

**关键方法**：
```python
from ads_engine import ADSEngine

engine = ADSEngine()
engine.connect(workspace, library, design)
ds_path, results = engine.run_simulation()
```

**返回格式**：
```python
results = {
    'SP1.SP': DataFrame(...),  # S 参数数据
    'Meas1': DataFrame(...)     # 测量方程数据
}
```

### 2. 网表解析器 (`netlist_parser.py`)

**功能**：
- 解析 opt/tune 变量
- 提取参数范围

**使用示例**：
```python
from netlist_parser import NetlistParser

variables = NetlistParser.parse_from_file("design.net")
print(NetlistParser.format_variable_table(variables))
```

### 3. 结果可视化 (`visualizer.py`)

**功能**：
- S 参数曲线图
- Smith 圆图
- 从 DataFrame 直接绘图

**使用示例**：
```python
from visualizer import ResultVisualizer

viz = ResultVisualizer(output_dir="./results/plots")
viz.plot_from_dataframe(df, freq_col='freq', 
                       data_cols=['S[1,1]', 'S[2,1]'])
```

### 4. LLM 接口 (`llm_interface.py`)

**功能**：
- 参数建议
- 优化策略
- 结果分析

**使用示例**：
```python
from llm_interface import LLMInterface

llm = LLMInterface(api_url="http://localhost:11434/api/generate")
suggestions = llm.suggest_initial_parameters(design_spec, variables)
```

---

## 🔧 工作流程

```
1. 连接 ADS
   ↓
2. 加载设计 → 生成网表 → 解析变量
   ↓
3. 运行仿真 → 使用 CircuitSimulator
   ↓
4. 提取结果 → .ds 文件 → DataFrame
   ↓
5. 绘制图表 → 保存 PNG
```

---

## 📊 仿真结果

**输出目录**：`./results/`

```
results/
├── design.net                  # 网表文件
├── variables.json              # 可调变量
├── baseline_results.json       # 仿真结果元数据
└── plots/                      # 图表
    ├── s_params_SP1.SP.png    # S 参数图
    └── measurements_*.png      # 测量值图
```

**结果数据格式**：
```python
# DataFrame 示例
df.columns = ['freq', 'S[1,1]', 'S[2,1]', 'groupdelay', ...]
```

---

## ⚙️ 配置说明

### config.json 完整配置

```json
{
  "_comment": "LLMRF 配置文件",
  
  "ads": {
    "_comment": "ADS 安装路径",
    "install_path": "F:/Program Files (x86)/ADS2026",
    "python_exe": "F:/Program Files (x86)/ADS2026/tools/python/python.exe"
  },
  
  "project": {
    "_comment": "项目信息（留空=自动检测）",
    "workspace": "",
    "library": "",
    "design": ""
  },
  
  "llm": {
    "_comment": "LLM 配置",
    "enabled": false,
    "api_url": "http://localhost:11434/api/generate",
    "model": "qwen2.5:14b"
  },
  
  "output": {
    "_comment": "输出设置",
    "dir": "./results",
    "save_plots": true
  }
}
```

---

## 🐛 常见问题

### Q1: 导入错误 - `keysight.ads.dds` 必须先导入

**错误信息**：
```
RuntimeError: "keysight.ads.dataset" cannot be imported without first importing "keysight.ads.dds"
```

**解决**：已修复，导入顺序为：
```python
from keysight.ads import dds      # 先导入 dds
from keysight.ads import dataset  # 再导入 dataset
```

### Q2: HPEESOF_DIR 路径错误

**错误信息**：
```
RuntimeError: HPEESOF_DIR is set to "..." but this path is not a directory
```

**解决**：
1. 运行 `check_config.py`
2. 修正 `config.json` 中的 `install_path`
3. 使用 `/` 而不是 `\`

### Q3: 没有生成图表

**原因**：可能缺少仿真数据或测量方程

**解决**：
1. 在 ADS 设计中添加 S_Param 控制器
2. 添加 MeasEqn 进行数据测量
3. 参考 `4.py` 示例

### Q4: 仿真无结果

**检查**：
1. 设计是否有效？
2. 仿真控制器是否配置？
3. 查看 `sim_output_*/` 目录日志

---

## 📝 代码示例

### 完整使用流程

```python
import os
os.environ['HPEESOF_DIR'] = "F:/Program Files (x86)/ADS2026"

from ads_engine import ADSEngine
from netlist_parser import NetlistParser
from visualizer import ResultVisualizer

# 1. 创建引擎
engine = ADSEngine(verbose=True)

# 2. 连接
engine.connect(
    workspace_path="D:/Desktop/test_wrk",
    library_name="test_lib",
    design_name="test"
)

# 3. 生成网表
netlist = engine.generate_netlist()

# 4. 解析变量
variables = NetlistParser.parse_variables(netlist)

# 5. 运行仿真
ds_path, results = engine.run_simulation(output_no=1)

# 6. 绘制结果
viz = ResultVisualizer(output_dir="./results/plots")
for block_name, df in results.items():
    viz.plot_from_dataframe(df, title=block_name)

# 7. 清理
engine.close()
```

### 参考 4.py 的数据提取

```python
# 仿真结果已经被提取为 DataFrame
for block_name, df in results.items():
    # 提取频率和 S 参数
    freq = df['freq'].values / 1e6  # 转换为 MHz
    
    if 'S[2,1]' in df.columns:
        S21 = 20 * np.log10(abs(df['S[2,1]'].values))
        S11 = 20 * np.log10(abs(df['S[1,1]'].values))
        
        # 绘图
        import matplotlib.pyplot as plt
        plt.figure()
        plt.plot(freq, S21, label='S21')
        plt.plot(freq, S11, label='S11')
        plt.xlabel('Frequency (MHz)')
        plt.ylabel('Magnitude (dB)')
        plt.legend()
        plt.grid()
        plt.savefig('s_params.png')
```

---

## 🔄 版本更新记录

### v2.0 (2026-02-02)
- ✅ 修复导入顺序问题（dds 先于 dataset）
- ✅ 改进仿真流程，使用 CircuitSimulator
- ✅ 添加 DataFrame 数据提取功能
- ✅ 参考 4.py 改进数据处理
- ✅ 简化项目结构，删除无用文件
- ✅ 创建统一项目文档

### v1.0
- 初始版本

---

## 📞 技术支持

**配置问题**：运行 `check_config.py`  
**功能问题**：查看本文档相关章节  
**错误日志**：查看终端输出和 `sim_output_*/` 目录

---

**所有修改请记录在本文档！**
